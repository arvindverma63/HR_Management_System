...
<body class="bg-gradient-to-br from-gray-50 to-gray-200 font-sans antialiased">
    <div class="flex min-h-screen">
        ...
        <div class="flex-1 p-3 md:p-6 w-full">
            <button id="open-sidebar" class="md:hidden fixed top-3 left-3 z-30 bg-custom-blue text-white p-1.5 rounded-md focus:outline-none">
            ...
            <section id="employee-additions">
                <div class="bg-white p-4 md:p-6 rounded-xl shadow-md">
                    ...
                    <form id="addition-form" method="POST" action="{{ isset($addition) ? route('additions.update', $addition->id) : route('additions.store') }}" class="mb-6">
                        @csrf
                        @if(isset($addition)) @method('PUT') @endif

                        <div class="mb-4">
                            <label for="location_id" class="block text-xs font-medium text-gray-700">Select Location</label>
                            <select id="location_id" name="location_id" class="mt-1 w-full md:w-48 p-1.5 border rounded-md focus:ring-1 focus:ring-custom-blue text-sm" required>
                        ...
                        <div id="employee-table-container" class="mb-4 hidden">
                            ...
                            @error('employee_ids')<span class="text-red-500 text-xs mt-1">{{ $message }}</span>@enderror
                        </div>
                        ...
                        <div id="additions-container" class="mb-4 hidden">
                            ...
                            <div id="additions-rows">
                                <div class="addition-row grid grid-cols-1 md:grid-cols-3 gap-3 mb-3">
                                    <div>
                                        ...
                                        @error('additions.0.type')<span class="text-red-500 text-xs mt-1">{{ $message }}</span>@enderror
                                    </div>
                                    <div>
                                        ...
                                        @error('additions.0.rate')<span class="text-red-500 text-xs mt-1">{{ $message }}</span>@enderror
                                    </div>
                            ...
                            <button type="button" onclick="addAdditionRow()" class="bg-green-500 text-white p-1.5 rounded-md hover:bg-green-600 transition duration-200 text-sm flex items-center gap-1" title="Add Another Addition">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div class="flex gap-2">
                            ...
                            @if(isset($addition))
                                <button type="button" onclick="resetForm()" class="bg-gray-500 text-white p-1.5 rounded-md hover:bg-gray-600 transition duration-200 text-sm flex items-center gap-1">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
                                    </svg>
                                    Cancel
                    ...
                    <!-- Filter by Location -->
                    <div class="mb-4">
                        <label for=" gets broken here location_filter" class="block text-xs font-medium text-gray-700">Filter by Location</label>
                        <select id="location_filter" class="mt-1 w-full md:w-48 p-1.5 border rounded-md focus:ring-1 focus:ring-custom-blue text-sm">
                            ...
                            <tbody>
                                @foreach ($additions as $addition)
                                    <tr class="hover:bg-gray-50">
                                        <td class="p-2 text-sm text-gray-700">{{ $addition->employee->name ?? 'N/A' }}</td>
            ...
            fetch(`/additions/${id}/edit`)
                ...
                .then(data => {
                    ...
                    if (!buttonContainer.querySelector('button[onclick="resetForm()"]')) {
                        ...
                        cancelButton.className = 'bg-gray-500 text-white p-1.5 rounded-md hover:bg-gray-600 transition duration-200 text-sm flex items-center gap-1';
