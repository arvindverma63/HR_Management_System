...
<body class="bg-gradient-to-br from-gray-50 to-gray-200 font-sans antialiased">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                                <div>
                                    <label for="location-name" class="block text-sm font-medium text-gray-700">Location
                                        Name</label>
                                    <input type="text" id="location-name" name="name" value="{{ old('name') }}"
                                ...
                                <div>
                                    <label for="contact-number" class="block text-sm font-medium text-gray-700">Contact
                                        Number</label>
                                    <input type="tel" id="contact-number" name="contact_number" value="{{ old('contact_number') }}"
                                        class="mt-1 w-full p-2 md:p-3 border rounded-lg focus:ring-2 focus:ring-custom-blue focus:border-custom-blue transition-all"
                                ...
                                <div>
                                    <label for="start-shift-time" class="block text-sm font-medium text-gray-700">Start Shift Time</label>
                                    <input type="time" id="start-shift-time" name="start_shift_time" value="{{ old('start_shift_time') }}"
                                        class="mt-1 w-full p-2 md:p-3 border rounded-lg focus:ring-2 focus:ring-custom-blue focus:border-custom-blue transition-all">
                                    @error('start_shift_time')
                                        <span class="text-red-500 text-sm mt-1">{{ $message }}</span>
                                    @enderror
                                </div>
                                <div>
                                    <label for="end-shift-time" class="block text-sm font-medium text-gray-700">End Shift Time</label>
                                    <input type="time" id="end-shift-time" name="end_shift_time" value="{{ old('end_shift_time') }}"
                                        class="mt-1 w-full p-2 md:p-3 border rounded-lg focus:ring-2 focus:ring-custom-blue focus:border-custom-blue transition-all">
                                    @error('end_shift_time')
                                        <span class="text-red-500 text-sm mt-1">{{ $message }}</span>
                                    @enderror
                                </div>
                            </div>
                            ...
                            <table class="w-full text-left text-sm md:text-base">
                                <thead>
                                    <tr class="bg-custom-blue text-white">
                                        ...
                                        <th class="p-2 md:p-4">Start Shift Time</th>
                                        <th class="p-2 md:p-4">End Shift Time</th>
                                ...
                                <tbody>
                                    @forelse ($locations as $location)
                                        <tr class="border-b hover:bg-gray-50 transition-all">
                                            ...
                                            <td class="p-2 md:p-4">{{ $location->address ?? 'N/A' }}</td>
                                            <td class="p-2 md:p-4">{{ $location->city ?? 'N/A' }}</td>
                                            <td class="p-2 md:p-4">{{ $location->state ?? 'N/A' }}</td>
                                            <td class="p-2 md:p-4">{{ $location->contact_number ?? 'N/A' }}</td>
                                            <td class="p-2 md:p-4">{{ $location->start_shift_time ?? 'N/A' }}</td>
                                            <td class="p-2 md:p-4">{{ $location->end_shift_time ?? 'N/A' }}</td>
                                            <td class="p-2 md:p-4 flex space-x-2">
                                                <button
                                                    onclick="openEditModal({{ $location->id }}, '{{ $location->name }}', '{{ $location->address }}', '{{ $location->city }}', '{{ $location->state }}', '{{ $location->contact_number }}', '{{ $location->start_shift_time }}', '{{ $location->end_shift_time }}')"
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="8" class="p-2 md:p-4 text-center text-gray-500">No locations
                                                found.</td>
                                        </tr>
                                    @endforelse
                                </tbody>
                ...
                <div class="bg-white p-6 rounded-2xl shadow-lg w-full max-w-lg">
                    ...
                    <form id="editForm" method="POST" class="grid grid-cols-1 gap-4 md:gap-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                            ...
                            <div>
                                <label for="edit-start-shift-time" class="block text-sm font-medium text-gray-700">Start Shift Time</label>
                                <input type="time" id="edit-start-shift-time" name="start_shift_time"
                                    class="mt-1 w-full p-2 md:p-3 border rounded-lg focus:ring-2 focus:ring-custom-blue focus:border-custom-blue transition-all">
                                @error('start_shift_time')
                                    <span class="text-red-500 text-sm mt-1">{{ $message }}</span>
                                @enderror
                            ...
                            <div>
                                <label for="edit-end-shift-time" class="block text-sm font-medium text-gray-700">End Shift Time</label>
                                <input type="time" id="edit-end-shift-time" name="end_shift_time"
                                    class="mt-1 w-full p-2 md:p-3 border rounded-lg focus:ring-2 focus:ring-custom-blue focus:border-custom-blue transition-all">
                                @error('end_shift_time')
                                    <span class="text-red-500 text-sm mt-1">{{ $message }}</span>
                                @enderror
    ...
    <script>
        ...
        function openEditModal(id, name, address, city, state, contact_number, start_shift_time, end_shift_time) {
            ...
            document.getElementById('edit-start-shift-time').value = start_shift_time || '';
            document.getElementById('edit-end-shift-time').value = end_shift_time || '';
