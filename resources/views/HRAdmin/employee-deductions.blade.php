...
<body class="bg-gradient-to-br from-gray-50 to-gray-200 font-sans antialiased">
    <div class="flex min-h-screen">
        ...
        <div class="flex-1 p-4 md:p-8 w-full">
            ...
            @if (session('success'))
                <div class="max-w-3xl mx-auto mb-4 px-4 py-3 rounded bg-green-100 text-green-800 border border-green-300 shadow-sm">
                    {{ session('success') }}
                </div>
            @endif

            @if (session('error'))
                <div class="max-w-3xl mx-auto mb-4 px-4 py-3 rounded bg-red-100 text-red-800 border border-red-300 shadow-sm">
                    {{ session('error') }}
                </div>
            @endif
            ...
            <div class="max-w-2xl mx-auto my-8 bg-white shadow-lg rounded-xl p-6">
                ...
                <form method="GET" action="{{ route('employee-deductions') }}" class="grid grid-cols-1 md:grid-cols-2 gap-4 items-end">
                    <div>
                        <label for="location_id" class="block text-sm font-medium text-gray-700 mb-1">Location</label>
                        <select id="location_id" name="location_id" class="w-full border-gray-300 rounded-lg shadow-sm p-3 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition" required>
                    ...
                    <button type="submit" class="bg-blue-600 text-white px-6 py-3 rounded-lg shadow hover:bg-blue-700 transition w-full md:w-auto">
                        Search
                    </button>
                </form>
            </div>
            ...
            @if ($selectedLocation)
                ...
                <div class="max-w-5xl mx-auto bg-white shadow-lg rounded-xl p-6">
                    <h3 class="text-xl font-semibold mb-4 text-gray-800">Employees</h3>
                                ...
                                <tbody class="divide-y divide-gray-200">
                                    @foreach ($employees as $employee)
                                        <tr class="hover:bg-gray-50 transition">
                                            ...
                                            <td class="px-6 py-4">{{ $employee->deductions->count() }} Deduction(s)</td>
                                            <td class="px-6 py-4 space-x-2">
                                                <button onclick="openAddDeductionModal('{{ $employee->id }}', '{{ $employee->name }}')"
                                                        class="bg-green-600 text-white px-3 py-1 rounded-lg hover:bg-green-700 transition">
                                                    Add Deduction
                                                </button>
                                                @if ($employee->deductions->isNotEmpty())
                                                    <button onclick="openDeductionsModal('{{ $employee->id }}', '{{ $employee->name }}')"
    ...
    <!-- Add Deduction Modal -->
    <div id="addDeductionModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 hidden flex items-center justify-center z-50">
        <div class="bg-white rounded-lg p-6 w-full max-w-md">
            <h3 class="text-lg font-semibold mb-4">Add Deduction for <span id="addDeductionEmployeeName"></span></h3>
            <form id="addDeductionForm" method="POST" action="{{ route('employee-deductions.store') }}">
                @csrf
                <input type="hidden" name="employee_id" id="addDeductionEmployeeId">
                <div class="mb-4">
                    <label for="addDeductionType" class="block text-sm font-medium text-gray-700 mb-1">Type</label>
                    <select id="addDeductionType" name="type" class="w-full border-gray-300 rounded-lg shadow-sm p-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition" required>
                ...
                <div class="mb-4">
                    ...
                    <input type="number" id="addDeductionRate" name="rate" step="0.01" class="w-full border-gray-300 rounded-lg shadow-sm p-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition" required>
                    @error('rate')
                        <span class="text-red-500 text-sm mt-1">{{ $message }}</span>
                    @enderror
                </div>
                <div class="flex justify-end space-x-2">
                    <button type="button" onclick="closeAddDeductionModal()" class="bg-gray-300 text-gray-700 px-4 py-2 rounded-lg hover:bg-gray-400 transition">Cancel</button>
    ...
    <!-- Deductions Modal -->
    <div id="deductionsModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 hidden flex items-center justify-center z-50">
        <div class="bg-white rounded-lg p-6 w-full max-w-4xl">
            <h3 class="text-lg font-semibold mb-4">Deductions for <span id="deductionsEmployeeName"></span></h3>
            <div id="deductionsTableContainer" class="overflow-x-auto">
                <!-- Deductions table will be populated via JavaScript -->
            </div>
            <div class="flex justify-end mt-4">
                <button onclick="closeDeductionsModal()" class="bg-gray-300 text-gray-700 px-4 py-2 rounded-lg hover:bg-gray-400 transition">Close</button>
            </div>
        </div>
    </div>

    <!-- Edit Deduction Modal -->
    <div id="editDeductionModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 hidden flex items-center justify-center z-50">
        <div class="bg-white rounded-lg p-6 w-full max-w-md">
            ...
            <form id="editDeductionForm" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <select id="editDeductionType" name="type" class="w-full border-gray-300 rounded-lg shadow-sm p-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition" required>
                        <option value="CASH">CASH</option>
                        <option value="MISC">MISC</option>
                        <option value="BANK ADV">BANK ADV</option>
                    </select>
                ...
                <div class="mb-4">
                    ...
                    <input type="number" id="editDeductionRate" name="rate" step="0.01" class="w-full border-gray-300 rounded-lg shadow-sm p-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition" required>
                    @error('rate')
                        <span class="text-red-500 text-sm mt-1">{{ $message }}</span>
                    @enderror
                </div>
                <div class="flex justify-end space-x-2">
                    <button type="button" onclick="closeEditDeductionModal()" class="bg-gray-300 text-gray-700 px-4 py-2 rounded-lg hover:bg-gray-400 transition">Cancel</button>
    ...
    <div id="deleteDeductionModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 hidden flex items-center justify-center z-50">
        <div class="bg-white rounded-lg p-6 w-full max-w-md">
            ...
            <form id="deleteDeductionForm" method="POST">
                ...
                <div class="flex justify-end space-x-2">
                    <button type="button" onclick="closeDeleteDeductionModal()" class="bg-gray-300 text-gray-700 px-4 py-2 rounded-lg hover:bg-gray-400 transition">Cancel</button>
    ...
    <script>
        ...
        function openAddDeductionModal(employeeId, employeeName) {
            document.getElementById('addDeductionEmployeeId').value = employeeId;
            document.getElementById('addDeductionEmployeeName').textContent = employeeName;
        ...
        function openDeductionsModal(employeeId, employeeName) {
            document.getElementById('deductionsEmployeeName').textContent = employeeName;
            const deductionsTableContainer = document.getElementById('deductionsTableContainer');
            const employees = @json($employees);
            const employee = employees.find(e => e.id == employeeId);
            const deductions = employee ? employee.deductions : [];
            let tableHtml = `
                <table class="min-w-full border border-gray-200 rounded-lg">
        ...
        function openEditDeductionModal(deductionId, type, rate) {
            ...
            document.getElementById('editDeductionForm').action = '{{ route("employee-deductions.update", ":id") }}'.replace(':id', deductionId);
        ...
        function openDeleteDeductionModal(deductionId) {
            ...
            document.getElementById('deleteDeductionForm').action = '{{ route("employee-deductions.destroy", ":id") }}'.replace(':id', deductionId);
